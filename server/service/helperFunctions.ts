...
const validate = (req:Request,helper : IUser) : string[] => {
  const err : string[] = []
   if (!helper.name || !helper.name.trim()) err.push('Name is required')
    ...
    if(!Object.values(vehicleTypesEnum).includes(helper.vehicleType as vehicleTypesEnum)) err.push('enter valid vehicle type')
